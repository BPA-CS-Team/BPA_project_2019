import pygame,timer, GameObject, Box2D, inventory, observer
...
class Player(GameObject.GameObject) :
 ...
 def is_falling(self):
  ...
  Return Value:
   True on success, False on failure"""
  return not self.jumping and self.falling
 ...
 def update_coordinates(self, val):
  """Updates the player's coordinates
  Parameters:
   val (int, 1 <= 4): The coordinate to be updated
  Return Value:
   None"""
  #Increment or decrement the coords silently, no audio or wall logic
  dx, dy = 0, 0
  if val == 1: dy += 1
  elif val == 2: dx += 1
  elif val == 3: dy -= 1
  ...
  self.body.ApplyLinearImpulse(impulse=(dx*self.body.mass*2, dy*self.body.mass*4), point=(self.body.position.x, self.body.position.y), wake=True)
 ...
 def fallingloop(self):
  ...
  if self.jumptimer.elapsed >= self.jumptime:
   ...
   #When the if statement will be added, this code will need to move up by 1 indention level (add 1 more space)
