...
class Player(GameObject):
	...
	def collision_begin(self, game_object):
		"""
		this is called when a collision has begun
		"""
		if game_object.image.get_at((0, 0)) == (237, 28, 36, 255):
			self.dead = True
		print('Player is now touching: '+game_object.__class__.__name__)

	def collision_end(self, game_object):
		print('Player is no longer touching: '+game_object.__class__.__name__)

	def post_collision_solve(self, game_object, impulse):
		"""
		called after a collision has been solved
		"""
	...
	def is_object_below(self, rect):
		"""
		checks to see if an object is below player, if yes returns 1, if no returns 0
		"""
	...
	def is_object_above(self, rect):
		"""
		checks to see if an object is above player, if yes returns 1, if no returns 0
		"""
	...
	def is_object_left(self, rect):
		"""
		checks if object is left, if yes returns 1, if no returns 0
		"""
	...
	def is_object_right(self, rect):
		"""
		checks if object is right, if yes returns 1, if no returns 0
		"""
